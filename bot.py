...
def result_menu() -> InlineKeyboardMarkup:
    keyboard = [
        [InlineKeyboardButton("📄 ЗАКАЗАТЬ ПОЛНЫЙ ОТЧЕТ", callback_data="order_full_report")],
        [InlineKeyboardButton("💳 ОПЛАТА", callback_data="payment")],
...
def tariff_menu() -> InlineKeyboardMarkup:
    keyboard = [
        [InlineKeyboardButton("⭐ БАЗОВЫЙ - 50₽/отчет", callback_data="tariff_basic")],
        [InlineKeyboardButton("💎 ПРЕМИУМ - 150₽/отчет", callback_data="tariff_premium")],
        [InlineKeyboardButton("👑 VIP - 500₽/месяц", callback_data="tariff_vip")],
        [InlineKeyboardButton("💳 ОПЛАТА", callback_data="payment")],
        [InlineKeyboardButton("⬅️ НАЗАД", callback_data="back_main")],
    ]
    return InlineKeyboardMarkup(keyboard)
...
3️⃣ Оплатите и закажите полный отчет
...
3️⃣ Оплата и получение полного отчета
...
💎 *ПРЕМИУМ - 150₽/отчет*
• Полный отчет
• Детализация по всем источникам
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "payment_rub":
        await query.edit_message_text(
            f"₽ *ОПЛАТА РУБЛЯМИ*\n\n"
            f"Отправьте:\n"
            f"• 199 рублей (1 отчет)\n"
            f"• 499 рублей (4 отчета)\n"
            f"• 899 рублей (16 отчетов)\n\n"
            f"📌 Номер карты/счета:\n`{RUB_CARD}`\n\n"
            f"🏦 Банк: {RUB_BANK}\n\n"
            f"✅ После оплаты нажмите кнопку «ОПЛАТИЛ»",
            parse_mode=ParseMode.MARKDOWN,
    ...
    if data == "payment_done":
        await query.edit_message_text(
            ...
            f"Пришлите пожалуйста подтверждение оплаты на email:\n\n"
            f"📧 `{SUPPORT_EMAIL}`\n\n"
...
def get_example(search_type: str) -> str:
    examples = {
        ...
        "email": "moscowkil@gmail.com", "vin": "WAUZZZ8V3JA123456",
